"""benchmarks.latency — mnelo recall latency benchmark.

[8/11 P3-benchmarks] 从 `scripts/benchmark.py` 整体迁移到 `benchmarks/latency.py`，
保持原有行为：seed 10k chunks + 100 queries + 测 p50/p95/p99/min/max/mean/stdev +
cleanup 幂等。

入口：
  python -m benchmarks latency                       # 默认 10k / 100q
  python -m benchmarks latency --chunks 50000       # 50k
  python -m benchmarks latency --chunks 5000 --json out.json

历史：原 `scripts/benchmark.py` 由 v0.5.5 (7/19) 引入，验证 README 引用的 p50=12.5ms
延迟数字。8/11 P3 任务卡要求把入口提升到 `python -m benchmarks <name>` 风格，对标
mem0 memory-benchmarks 的 public evaluation harness 模式。
"""
...
def build_parser() -> argparse.ArgumentParser:
    """Build the latency benchmark argument parser."""
    parser = argparse.ArgumentParser(
        prog="python -m benchmarks latency",
        description="mnelo recall latency benchmark (vector + graph + meta + entity)",
    )
    parser.add_argument(
        "--chunks",
        type=int,
