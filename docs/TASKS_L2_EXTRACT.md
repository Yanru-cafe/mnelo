...
> **给 hermes 的执行指南**。实现 DESIGN §5.2 **P1a（规则提取）**的第一步——**memory_type 规则分类器**，解决实战发现的"4344/4344 chunks 100% fact，6 类系统空架子"问题（v0.3 报告 §2）。
> **语言覆盖**：简体中文 + **繁体中文** + 英文。
...
**问题**：`remember()` 默认 `memory_type='fact'`，调用方（Hermes agent 对话，68% 来源）从不传类型 → **实战 0 个非 fact**。6 套 TTL（§3.0.5）+ 6 套矛盾规则 + H3 卫生全部空转。
...
> 标记是 **seed**，实施时按实战数据校准。**强标记** = 命中即分类；**弱标记** = 不单独分类（保持 fact）。
...
- v0.3 实战报告 §2（memory_type 100% fact 问题）
