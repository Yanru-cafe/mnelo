...
def test_s6_client_handles_disabled_digest_response():
    ...
    client._call = lambda *a, **kw: {
        "enabled": False, "content": "", "line_refs": {},
        "truncated": False, "built_at": None,
    }
...
def test_s6_server_dispatcher_handles_invalid_ref_gracefully():
    ...
    # Patch Memory.get_digest 抛内部错误, 模拟 dispatcher 兜底路径
    with patch.object(mcp_server, "_get_mem") as mock_get_mem:
        fake_mem = MagicMock()
        fake_mem.get_digest = MagicMock(side_effect=ValueError(
            "sqlite3.OperationalError: database is locked at /Users/apple/.hermes/memory/memory.db line 4521"
        ))
        ...
        assert data.get("detail") is None, (
            f"detail 不应暴露路径/堆栈, got {data.get('detail')!r}"
        )
        assert "memory.db" not in result_json, (
            f"内部路径不应泄露, got {result_json!r}"
        )
...
def test_s6_client_retry_on_transient_connection_refused():
    ...
    with patch.object(client, "_ensure_mcp", return_value=(fake_session_cls, fake_sse_client)), \
         patch.object(client, "_async_call", side_effect=fake_async_call):
...
def test_s6_client_retry_exhausted_raises_last_error():
    ...
    fake_sse_client = MagicMock()
    with patch.object(client, "_ensure_mcp", return_value=(fake_session_cls, fake_sse_client)), \
         patch.object(client, "_async_call", side_effect=always_fail):
...
def test_s6_client_sse_mid_stream_broken_pipe_raises():
    ...
    fake_sse_client = MagicMock()
    with patch.object(client, "_ensure_mcp", return_value=(fake_session_cls, fake_sse_client)), \
         patch.object(client, "_async_call", side_effect=mid_stream_fail):
