...
def test_p92_list_entities_returns_filtered(mem):
    ...
    # insert 3 entities
    for i, (eid, kind) in enumerate([("stock:sh600089", "stock"),
                                     ("stock:sh600090", "stock"),
                                     ("identity:agent:main", "identity_fact")]):
        mem._conn.execute(
            "INSERT INTO entities (id, kind, name, importance, valid_from) "
            "VALUES (?, ?, ?, ?, ?)",
...
def test_p92_list_entities_filters_soft_deleted(mem):
    ...
    from memory import now as _now
    mem._conn.execute(
        "INSERT INTO entities (id, kind, name, importance, valid_from, valid_until) "
        "VALUES (?, ?, ?, ?, ?, ?)",
...
def test_p92_search_relations_by_type(mem):
    ...
    from memory import now as _now
    mem._conn.execute(
        "INSERT INTO relations (source_id, target_id, relation, weight, valid_from, valid_until) "
        "VALUES (?, ?, ?, ?, ?, NULL)",
