"""[audit 修真痛点 #7 2026-08-16] user_id / run_id filter recall 完全未实现.

Owner 修真痛点优先级 #1 (跨用户数据泄漏 = production 安全 P1).
...
# === 修真痛点 #7: user_id filter recall ===

def test_user_id_filter_isolates_users(mem):
    """修真痛点 #7: filter user_id='alice' 应该只返 alice, 不返 bob."""
    _seed_two_users(mem)
    ...
    assert len(bob_leaked) == 0, (
        f"修真痛点 #7 CONFIRMED: user_id='alice' filter 漏返 bob 内容: {bob_leaked}"
    )
    # 必须返 alice
...
def test_run_id_filter_isolates_runs(mem):
    ...
    assert len(bob_leaked) == 0, (
        f"修真痛点 #7 CONFIRMED: run_id='bob_run_1' filter 漏返 alice 内容: {bob_leaked}"
    )
...
def test_legacy_chunks_without_user_id_excluded_when_filter_active(mem):
    ...
    这是修真设计 — 修真痛点 #7 fix 后, 没打 user_id tag 的 chunk 不被泄漏给 user_id filter.
    """
    ...
    assert len(legacy_leaked) == 0, (
        f"修真痛点 fix 后, 旧数据 (无 user_id tag) 不应被 user_id filter 召回: {legacy_leaked}"
    )
