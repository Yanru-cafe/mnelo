...
def test_e2_manual_fts5_sync_on_remember(mem):
    ...
    row = mem._conn.execute(
        "SELECT c.id, c.rowid FROM chunks c WHERE c.id = ?", (cid,)
    ).fetchone()
    chunk_rowid = row["rowid"]
    fts_rows = mem._conn.execute(
        "SELECT rowid FROM chunks_fts WHERE rowid = ?", (chunk_rowid,)
    ).fetchall()
...
def test_e2_short_chinese_falls_back_to_like(mem):
    ...
    assert any(r["chunk_id"] == cid for r in results), (
        f"LIKE fallback \u672a\u8fd4\u56de\u9884\u671f chunk\uff1a{results}"
    )
...
def test_e2_full_chinese_match_bm25(mem):
    ...
    results = mem._meta_recall(
        "\u4e70\u5165 \u592a\u9633\u80a1", top_k=5, filters={}, asof=None
    )
...
def test_e2_meta_recall_unions_fts5_and_like(mem):
    ...
    cid_long = mem.remember(
        "\u4e70\u5165 \u592a\u9633\u80a1 \u80a1\u4ef7\u4e0a\u5347", source="e2_test", importance=0.5
    )
    cid_short = mem.remember(
        "\u7279\u53d8 \u5176\u4ed6\u5185\u5bb9", source="e2_test", importance=0.5
    )
...
def test_e2_soft_delete_keeps_fts_row_filtered_by_valid_until(mem):
    ...
    mem._conn.execute(
        "UPDATE chunks SET valid_until = ? WHERE id = ?", (_now(), cid)
    )
    mem._conn.commit()
    # FTS5 \u5e94\u4ecd\u542b\u8be5 chunk \u00b7 \u4f46 recall \u8fc7\u6ee4\u4f1a\u8fc7\u6ee4
    results = mem._meta_recall(
        "soft delete", top_k=5, filters={}, asof=None
    )
    found_ids = [r["chunk_id"] for r in results]
    ...
    fts_rows = mem._conn.execute(
        "SELECT rowid FROM chunks_fts WHERE rowid = ?", (rowid,)
    ).fetchall()
...
def test_e2_no_chunks_fts_trigger_p81_safety(mem):
    ...
    triggers = mem._conn.execute(
        ...
        "AND (name LIKE 'trg_chunks_fts%' OR tbl_name = 'chunks_fts')"
    ).fetchall()
    assert len(triggers) == 0, (
        f"chunks_fts triggers \u4e0d\u5e94\u5b58\u5728 (P1 #81 SIGSEGV \u539f\u56e0)\uff1a{triggers}"
    )
...
def test_e2_meta_recall_filters_by_user_id(mem):
    ...
    results = mem._meta_recall(
        "filter test", top_k=5, filters={"user_id": "alice"}, asof=None
    )
    found_ids = [r["chunk_id"] for r in results]
