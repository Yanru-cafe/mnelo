...
def _setup(m):
    ...
    try:
        m._conn.execute(
            "DELETE FROM task_states WHERE task_id LIKE 'task:tlm4-%' "
            "OR task_id LIKE 'loop:tlm4-%'"
        )
        m._conn.execute(
            "DELETE FROM entities WHERE id LIKE 'task:tlm4-%' "
...
def test_loop_tick_dormant_when_disabled():
    ...
    try:
        _setup(m)
        lid = _mk_loop(m, "dormant", {
            "trigger": "x", "interval_hours": 24, "enabled": False,
        })
...
def test_loop_tick_waiting_when_active_task_open():
    ...
    try:
        ...
        lid = _mk_loop(m, "waiting", {
            "trigger": "x", "interval_hours": 24,
            "enabled": True, "active_task_id": tid,
        })
        r = ts_mod.loop_tick(m._conn, loop_id=lid, now="2026-08-06T11:00")
...
def test_loop_tick_due_first_run():
    ...
    try:
        _setup(m)
        lid = _mk_loop(m, "first", {
            "trigger": "x", "interval_hours": 24,
            "enabled": True, "active_task_id": None,
        })
        r = ts_mod.loop_tick(m._conn, loop_id=lid, now="2026-08-06T11:00")
